...
module.exports.connect = async (event, context) => {
  // console.log('connect:');
  if (event.queryStringParameters && event.queryStringParameters.exception)
    throw NaN
  if (event.queryStringParameters && event.queryStringParameters.return)
    return { statusCode: parseInt(event.queryStringParameters.return, 10) }
  if (
    event.queryStringParameters &&
    event.queryStringParameters['do-not-answer']
  )
    return // eslint-disable-line consistent-return

  const listener = await ddb
    .get({ TableName: 'data', Key: { name: 'default' } })
    .promise()
