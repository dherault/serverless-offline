...
describe('handler payload tests', () => {
  ...
  test.skip('should receive correct call info (event only)', async () => {
    ...
    expect(connect).toEqual({ action:'update', event:'connect', info:expectedCallInfo })
    expect(connect.info.event.requestContext.requestTimeEpoch).toBeWithinRange(connect.info.event.requestContext.connectedAt - 10, connect.info.event.requestContext.requestTimeEpoch + 10)
    expect(connect.info.event.requestContext.connectedAt).toBeWithinRange(now - timeout, now)
    expect(connect.info.event.requestContext.requestTimeEpoch).toBeWithinRange(now - timeout, now)
    expect(moment.utc(connect.info.event.requestContext.requestTime, 'D/MMM/YYYY:H:m:s Z').toDate().getTime()).toBeWithinRange(now - timeout, now)
    ...
    expect(callInfo).toEqual({ action:'update', event:'call-info', info:expectedCallInfo })
    expect(callInfo.info.event.requestContext.connectedAt).toBeLessThan(callInfo.info.event.requestContext.requestTimeEpoch)
    expect(callInfo.info.event.requestContext.connectedAt).toBeWithinRange(now - timeout, now)
    expect(callInfo.info.event.requestContext.requestTimeEpoch).toBeWithinRange(now - timeout, now)
    expect(moment.utc(callInfo.info.event.requestContext.requestTime, 'D/MMM/YYYY:H:m:s Z').toDate().getTime()).toBeWithinRange(now - timeout, now)
