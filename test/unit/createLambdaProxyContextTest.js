...
describe('createLambdaProxyContext', () => {
  ...
  context('with a GET /fn1 request', () => {
    ...
    it('should have a unique requestId', () => {
      ...
      ).to.be.greaterThan(prefix.length);
  ...
  context(
    'with a POST /fn1 request with a lowercase Content-Type header',
    () => {
  ...
  context(
    'with a POST /fn1 request with a camelcase Content-Type header',
    () => {
      it('should assign the value to Content-Type', () => {
        const requestBuilder = new RequestBuilder('POST', '/fn1');
        requestBuilder.addBody({ key: 'value' });
  ...
  context('with a POST /fn1 request with a X-GitHub-Event header', () => {
    it('should assign not change the header case', () => {
      ...
      expect(lambdaProxyContext.multiValueHeaders['X-GitHub-Event']).to.deep.eq(
        ['test'],
      );
    });
  });
  ...
  context('with a POST /fn1 request with multiValueHeaders', () => {
    it('should assign not change the header case', () => {
      ...
      expect(lambdaProxyContext.multiValueHeaders['Some-Header']).to.deep.eq([
        'test1',
        'test2',
      ]);
    });
  ...
  context(
    'with a GET /fn1?param=1 request with single parameter in query string',
    () => {
  ...
  context(
    'with a GET /fn1?param=1&param2=1 request with double parameters in query string',
    () => {
      const requestBuilder = new RequestBuilder('GET', '/fn1?param=1');
      requestBuilder.addQuery('param', '1');
  ...
  context(
    'with a GET /fn1?param=1&param=1 request with single query string',
    () => {
      const requestBuilder = new RequestBuilder('GET', '/fn1?param=1');
  ...
  context(
    'with a GET /fn1?param=1&param=2 request with multiValueQueryStringParameters',
    () => {
      ...
      it('multi value param should have a two values', () => {
        expect(
          Object.keys(lambdaProxyContext.multiValueQueryStringParameters)
            .length,
        ).to.eq(1);
        expect(
  ...
  context(
    'with a request that includes cognito-authentication-provider header',
    () => {
      const requestBuilder = new RequestBuilder('GET', '/fn1');
  ...
  context('with environment variables', () => {
    ...
    it('should have the expected accountId', () => {
      expect(lambdaProxyContext.requestContext.identity.accountId).to.eq(
        'customAccountId',
      );
    });
    ...
    it('should have the expected caller', () => {
      expect(lambdaProxyContext.requestContext.identity.caller).to.eq(
        'customCaller',
      );
    });

    it('should have the expected apiKey', () => {
      expect(lambdaProxyContext.requestContext.identity.apiKey).to.eq(
        'customApiKey',
      );
    });
